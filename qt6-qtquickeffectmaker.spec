...
Source:		qtquickeffectmaker-%{?snapshot:%{snapshot}}%{!?snapshot:%{version}}.tar.zst
%else
Source:		http://download.qt-project.org/%{?beta:development}%{!?beta:official}_releases/qt/%(echo %{version}|cut -d. -f1-2)/%{version}%{?beta:-%{beta}}/submodules/qtquickeffectmaker-everywhere-src-%{version}%{?beta:-%{beta}}.tar.xz
%endif
Group:		System/Libraries
Summary:	Qt %{major} Quick EffectMaker plugin
BuildRequires:	cmake
BuildRequires:	ninja
...
%description
Qt %{major} Quick effectmaker plugin

%package examples
Summary:	Examples demonstrating the use of %{name}
Group:		Documentation
...
%prep
%autosetup -p1 -n qtquickeffectmaker%{!?snapshot:-everywhere-src-%{version}%{?beta:-%{beta}}}
